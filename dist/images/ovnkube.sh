...
ready_to_start_node () {
  ...
  ovn_master_host=$(kubectl --token=$(cat /var/run/secrets/kubernetes.io/serviceaccount/token) get ep -n openshift-ovn-kubernetes ovnkube-master 2>/dev/null | grep 6642 | sed 's/:/ /' | awk '/ovnkube-master/{ print $2 }')
